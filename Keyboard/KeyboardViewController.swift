...
//      var capsWasSet = self.setCapsIfNeeded()
  ...
  override func viewDidLayoutSubviews() {
    ...
    let orientationSavvyBounds = CGRect(x: 0, y: 0, width: self.view.bounds.width, height: height)
...
//                keyView.addTarget(keyView, action: #selector(hidePop), forControlEvents: [.TouchDragExit, .TouchCancel])
            ...
            if key.hasOutput {
              keyView.addTarget(self, action: #selector(symbolKeyDown(_:)), for: .touchDown)
              keyView.addTarget(self, action: #selector(symbolKeyUp(_:)), for: .touchUpInside)
  ...
  func symbolKeyDown(_ sender: KeyboardKey) {
  }
  
  func symbolKeyUp(_ sender: KeyboardKey) {
  }
